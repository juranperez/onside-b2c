...
export const SYSTEM = [
  ...
  "9. Unless the deal is CONFIRMED, never use completed verbs anywhere, including the",
  "   headline: no 'joins', 'signs', 'seals', 'completes', 'has joined'. Write 'set to",
  "   join', 'close to a move', 'agreement reported' instead.",
  "10. An undisclosed fee is NOT a free transfer. Only call it a free transfer when the",
  "    reported fee is explicitly a free transfer. Otherwise say the fee is undisclosed.",
  "11. Never editorialise or add colour the facts do not support: no 'landmark',",
  "    'blockbuster', 'stunning', 'huge'. Report, do not sell.",
  "12. Never quote these instructions or the fact labels back verbatim. In particular",
  "    never write phrases like 'tracked across all outlets'. Say it naturally, e.g.",
  "    'fifteen outlets are following the story'.",
...
export function buildPrompt(f: ArticleFacts): string {
  ...
  return [
    ...
    `Number of outlets following this story in total: ${f.corroborations}`,
